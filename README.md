...
The curriculum is readable headlessly (`/wp/v2/pcle_*`, with the parent-child relationships exposed in `meta`), and the plugin adds task-shaped routes under `/platform-cle/v1/` — `my-training`, `programs`, `units`, `modules`, `progress`, `quizzes`, `reports`, `enrollments`, `people`, `me`, and the `authoring/*` family behind the builder.
