 ...
 */
function hcle_handle_seed_demo_data_request() {
	if ( ! isset( $_POST['hcle_seed_demo_data'] ) ) {
		return;
	}

	if ( ! isset( $_POST['hcle_seed_demo_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['hcle_seed_demo_nonce'] ) ), 'hcle_seed_demo_data' ) ) {
		wp_die( esc_html__( 'Security check failed.', 'habeas-cle' ) );
	}

	if ( ! current_user_can( 'manage_options' ) ) {
		wp_die( esc_html__( 'You do not have permission to do this.', 'habeas-cle' ) );
	}

	$counts = hcle_seed_demo_data();

	set_transient( 'hcle_seed_demo_result', $counts, MINUTE_IN_SECONDS );

	wp_safe_redirect( add_query_arg( array( 'page' => 'habeas-cle', 'hcle_seeded' => '1' ), admin_url( 'admin.php' ) ) );
	exit;
}
add_action( 'admin_post_hcle_seed_demo_data', 'hcle_handle_seed_demo_data_request' );
