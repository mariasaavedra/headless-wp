...
- 🟡→✅ No automated tests. **Fixed** for the plugin (`tests/smoke-test.php`,
  501 assertions across 32 sections). `apps/web` still has none: CI lints and
...
| 5 | Smoke tests on access-control, progress, files, REST | ✅ done (`tests/smoke-test.php`, 501 assertions across 32 sections, dependency-free); green in CI on every push |
