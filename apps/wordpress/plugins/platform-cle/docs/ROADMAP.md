...
- 🟡→✅ No automated tests. **Fixed** for the plugin (`tests/smoke-test.php`,
  530 assertions across 33 sections). `apps/web` still has none: CI lints and
...
| 5 | Smoke tests on access-control, progress, files, REST | ✅ done (`tests/smoke-test.php`, 530 assertions across 33 sections, dependency-free); green in CI on every push |
